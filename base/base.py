...
__all__ = ['Critical', 'Error', 'Warning', 'Info', 'echo', 'cli', 'error', 'warning', 'debug', 'info', 'log',
           'log_json', 'log_table', 'critical', 'fatal', 'add_init', 'init_all', 'sleep', 'set_output_filter', 'ws',
           'commentjson', "md5", "md5b", "compare_versions", "match_version", "int32_version", "version_int32"]
...
# cmd_type is used to define the variable if the inita are execute fot the ztc or the zdm cli
def init_all(cli_type="ztc"):
    for ii in _init_fns:
        ii(cli_type)
...
class ZCliContext(object):
    def __init__(self):
        ...
        from adm.client.client import ZdmClient
        from base.cfg import env
        self.adm_client = ADMClient(workspace_url=env.adm.workspaces,
                                    device_url=env.adm.devices,
...
@click.option("--traceback/--no-traceback", "notraceback", default=False,
              help="To enable/disable exception traceback printing on criticals.")
