...
# Pass a parameter (to be passed in init_all() in the main of the cli) to tell if the config are for the ztc or for the zdm

def init_cfg(cli_type="ztc"):
    # cmd_type = "ztc" | "zdm"
    # platform
    env.load_skin()
    ...
    if testmode == 1:
        ...
        env.connector = os.environ.get("ZERYNTH_ADM_URL", "https://api.adm.zerinth.com")
        ...
        env.connector = os.environ.get("ZERYNTH_ADM_URL", "https://api.zdm.stage.zerynth.com")
    ...
    if cli_type == "zdm":
        sso_params = "?redirect=" + env.connector + "/v1/login/ztc/"
    ...
    env.api = Var({
        ...
        # "sso":env.backend+"/sso",
        "sso": env.backend + "/sso" + sso_params,
