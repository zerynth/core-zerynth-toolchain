...
class viper_cobj():
    ...
    def finalize(self,table):
        ...
        for k,v in table.table.items():
            ...
            elif v[3]==".bss" and v[0]=="__bss_end__":
                bss_end = int(v[2],16)
...
class gcc():
    def __init__(self,tools, opts={},logger=None):
        ...
        self.objdump = tools["arm"]["objdump"]
        self.ld = tools["arm"]["ld"]
        self.readelf = tools["arm"]["readelf"]
        self.logger=logger
    ...
    def run_command(self,cmd, args):
        ...
        try:            
            ...
            ret = e.returncode
        if self.logger:
            self.logger.info("ARMGCC: RET %s RUNNING %s, GOT %s",str(ret)," ".join(torun),output)
        return (ret,output)
